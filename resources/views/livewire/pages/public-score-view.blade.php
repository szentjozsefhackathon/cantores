...
<div class="py-8">
    <div class="mx-auto max-w-5xl xl:max-w-7xl px-4 sm:px-6 lg:px-8">
        @if($isPreview)
        <div class="mb-4 flex flex-wrap items-center gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950/40">
            <flux:icon name="eye" variant="mini" class="shrink-0 text-amber-600" />
            <flux:text class="flex-1 text-sm">
                {{ __('Editor preview: this nomination is :status and nobody else can see this page.', ['status' => $this->publication->status->label()]) }}
            </flux:text>
            <flux:button size="sm" variant="outline" :href="route('score-publication-review', ['publicationId' => $this->publication->id])">
                {{ __('Back to the review queue') }}
            </flux:button>
        </div>
        @endif
        ...
        <flux:card class="p-4 lg:p-6">
            <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                    ...
                    @if($this->score->music)
                    <flux:subheading>
                        <a href="{{ route('music-view', $this->score->music) }}" class="hover:underline">{{ $this->score->music->title }}</a>
                    </flux:subheading>
                    @endif
                </div>
...
<script src="{{ \App\Support\VendorAsset::url('js/abc2svg-1.js') }}"></script>
            ...
            <div class="mb-6 space-y-2">
                ...
                <div wire:key="public-score-file-{{ $file->id }}" class="flex flex-wrap items-center gap-2">
                    @if($file->isReady())
                    <x-score-file-pages
                        name="score-file-pages-public-{{ $file->id }}"
                    ...
                    <flux:button
                        ...
                        variant="outline"
                        size="sm"
                        :href="route('public-scores.file.download', ['score' => $this->score, 'scoreFile' => $file])">
            ...
            >
                {{-- Reading comes first: the display controls stay one click away. --}}
                <div x-data="{ showSettings: false }" class="mb-4">
                    <flux:button
                        size="sm"
                        variant="ghost"
                        icon="adjustments-horizontal"
                        x-on:click="showSettings = ! showSettings"
                        x-bind:aria-expanded="showSettings ? 'true' : 'false'"
                    >
                        {{ __('Display settings') }}
                    </flux:button>

                    <div x-show="showSettings" x-cloak class="mt-2">
                        ...
                        <div x-show="$wire.format === 'chordpro'" x-cloak class="flex flex-wrap items-center gap-x-3 gap-y-2 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 dark:border-zinc-700 dark:bg-zinc-800/50">
                            ...
                            <flux:tooltip :content="__('Font')">
                                <div class="flex items-center gap-1">
                                    <flux:icon name="type-outline" variant="micro" class="shrink-0 text-zinc-500 dark:text-zinc-400" />
                                    <x-lyric-font-select model="chordproFontFamily" width="w-32" />
                                </div>
                            </flux:tooltip>
                        ...
                        <div x-show="$wire.format === 'abc'" x-cloak class="flex flex-wrap items-center gap-x-3 gap-y-2 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 dark:border-zinc-700 dark:bg-zinc-800/50">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <flux:input size="sm" type="number" x-model="abcLyricFirstSkip" min="0" max="3" step="0.1" placeholder="1.1" class="w-16" />
                            </div>
            ...
            <div class="mt-8 border-t border-zinc-200 pt-6 dark:border-zinc-700">
                <x-score-attribution :publication="$this->publication" :reportable="! $isPreview" />
            </div>
