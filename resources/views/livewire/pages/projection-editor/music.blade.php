{{-- One music of the deck: the plan's, or one only this deck holds.
...
>
    <div class="flex items-center gap-1.5 text-sm">
        ...
        <div class="flex min-w-0 flex-1 items-center gap-1 {{ $musicChosen ? 'font-medium' : 'text-zinc-500 dark:text-zinc-400' }}">
            ...
            @if($local)
                <flux:badge size="sm" color="amber" class="shrink-0">{{ __('Only in this projection') }}</flux:badge>
            @endif

            {{-- The music's own name is shown above it where a slot holds several,
                 beside the slot where it holds one. Either way this is the switch
                 that keeps it off the screen, next to the name it governs. --}}
            @if($musicChosen)
            ...
            {{-- Where the music is to be found in the books the congregation
                 ...
                 switch, and it is drawn as it will be shown — solid once the deck
                 says it, outlined and pale while it does not. --}}
            @if($music['reference'])
                @if($musicChosen)
                ...
                @else
                    {{-- Nothing of this music is in the deck, so there is no row
            ...
            @if($local)
                ...
                @if($musicChosen)
                    <flux:modal.trigger :name="'remove-added-music-'.$music['id']">
                        <flux:tooltip :content="__('Remove this music from the projection')">
                            <flux:button size="sm" variant="ghost" icon="trash" :aria-label="__('Remove this music from the projection')" />
                        </flux:tooltip>
                    </flux:modal.trigger>
                    ...
                    <flux:modal :name="'remove-added-music-'.$music['id']" class="min-w-[22rem]">
                        <div class="space-y-6">
                            <div>
                                <flux:heading size="lg">{{ __('Remove this music from the projection') }}</flux:heading>
                ...
                @else
                    <flux:tooltip :content="__('Remove this music from the projection')">
                        <flux:button size="sm" variant="ghost" icon="trash" :aria-label="__('Remove this music from the projection')" wire:click="removeAddedMusic({{ $music['id'] }})" />
                    </flux:tooltip>
                @endif
            @endif
        </div>
