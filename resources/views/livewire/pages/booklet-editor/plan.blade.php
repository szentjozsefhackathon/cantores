...
<flux:card class="p-4">
    <div class="mb-3 flex items-center justify-between gap-2">
        ...
        <div class="flex shrink-0 items-center gap-1">
            ...
            <flux:tooltip :content="__('Add a music only this booklet holds, at its end')">
    ...
    @if($this->outline === [])
        <flux:text class="text-sm text-zinc-500">
            {{ $booklet->musicPlan
                ...
                : __('This booklet has no music plan yet. Add words or music here, or add a plan to choose from a service.') }}
        </flux:text>
    @endif
    ...
    {{-- The one search for a music the plan does not have, wherever it was
         ...
         open. Lazy, so a booklet nobody adds music to never loads it. --}}
    <flux:modal name="booklet-add-music" class="w-full max-w-4xl">
        <div class="space-y-4">
            <div>
                <flux:heading size="lg">{{ __('Add music') }}</flux:heading>
                <flux:subheading>{{ __('Only this booklet holds it; the plan stays as it is.') }}</flux:subheading>
            </div>

            <livewire:music-search lazy selectable="true" source="-booklet" wire:key="booklet-add-music-search" />
    ...
    <ul class="booklet-plan space-y-1.5">
        @foreach($this->outline as $node)
            ...
            @elseif($node['kind'] === 'music')
                {{-- A music this booklet holds between slots: after the homily, for
                     a birthday. It moves across whole slots with its arrows. --}}
                @include('livewire.pages.booklet-editor.music', ['music' => $node])
            @else
                ...
                >
                    ...
                    <div class="flex items-center gap-1.5 rounded-md px-2 py-1 {{ $slotChosen ? 'bg-green-50 dark:bg-green-950/40' : 'bg-zinc-100 dark:bg-zinc-800' }}">
                        ...
                        <div class="flex shrink-0 items-center gap-0.5">
                            ...
                            {{-- A music the plan does not have, for this booklet alone:
                                 added at the end of the slot. --}}
                            <flux:tooltip :content="__('Add music to this slot, only in this booklet')">
                                <flux:button size="sm" variant="ghost" icon="music-plus" :aria-label="__('Add music to this slot, only in this booklet')" wire:click="startAddingMusic({{ $node['id'] }})" />
                            </flux:tooltip>
                    ...
                    <ul class="mt-1 ms-1 space-y-1.5">
                        @foreach($node['children'] as $child)
                            ...
                            @else
                                @include('livewire.pages.booklet-editor.music', ['music' => $child])
                            @endif
                        @endforeach
