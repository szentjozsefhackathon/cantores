...
>
    ...
    <flux:modal wire:model="showAddSlotModal">
        ...
        <form wire:submit="addSlot" class="space-y-4">
            ...
            <flux:field>
                <flux:label for="sequence">{{ __('Sequence Number') }} *</flux:label>
                <flux:input 
                    id="sequence" 
                    type="number" 
                    wire:model="sequence" 
                    ...
                    required
                />
                <flux:help>{{ __('Determines the order of slots in the template') }}</flux:help>
                @error('sequence')
                    <flux:error>{{ $message }}</flux:error>
                @enderror
            </flux:field>
            ...
            <flux:field>
                ...
                >
                    {{ __('Included by default in music plans') }}
                </flux:checkbox>
                <flux:help>{{ __('If unchecked, this slot will be marked as advanced/optional') }}</flux:help>
                @error('is_included_by_default')
                    <flux:error>{{ $message }}</flux:error>
                @enderror
            </flux:field>
    ...
    <flux:modal wire:model="showEditSlotModal">
        ...
        <form wire:submit="updateSlot" class="space-y-4">
            ...
            <flux:field>
                ...
                <flux:help>{{ __('If unchecked, this slot will be marked as advanced/optional') }}</flux:help>
                @error('edit_is_included_by_default')
                    <flux:error>{{ $message }}</flux:error>
                @enderror
            </flux:field>
