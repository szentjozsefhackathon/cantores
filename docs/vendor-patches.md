...
The abc2svg changes below live in the `@cantoreshu/abc2svg` package. Upgrade
that package when updating the renderer; its `FORK.md` records the maintained
changes.
...
## abc2svg — `@cantoreshu/abc2svg/abc2svg-1.js`

- Upstream: <https://chiselapp.com/user/moinejf/repository/abc2svg> (LGPL3+)
- Packaged fork: `@cantoreshu/abc2svg@1.0.0` (upstream `v1.23.5`)
- License: LGPL3+ — modifications must stay under the same terms.

### Fork change: `huchords` — Hungarian chord-symbol spelling

**What:** A small self-registering module concatenated into the forked build.
It is not upstream code because it has to run inside the engraver.
...
**Maintenance:** The fork keeps this module as long as the core calls
`abc2svg.mhooks.*` and passes chord symbols through `Abc.prototype.gch_build`
with `s.a_gch[i].text`.
...
**Maintenance:** When updating the fork from upstream, retain these three
changes unless upstream supplies an equivalent parameter.
...
**Maintenance:** When updating the fork from upstream, retain these six
changes. If upstream has split the first line out, preserve the placement
behaviour from sites 3, 5 and 6 over its equivalent implementation.
