# Revised Roles and Permissions Implementation Plan

## Database Seeder Implementation (Revised)
...
class RolePermissionSeeder extends Seeder
{
    ...
    private function assignPermissionsToRoles(): void
    {
        ...
        // Editor permissions (can CRUD their own content)
        $editorPermissions = [
...
### Example: CollectionPolicy Update
...
public function update(User $user, Collection $collection): bool
{
    ...
    // Check if user has permission to update collections
    if (!$user->hasPermissionTo('collection.update')) {
        return false;
    }
...
test('viewer can view music but not edit', function () {
    $viewer = User::factory()->create();
    $viewer->assignRole('viewer');
    $music = Music::factory()->create();
    
    $this->actingAs($viewer)
        ->get(route('music.show', $music))
        ->assertSuccessful();
        
    $this->actingAs($viewer)
        ->put(route('music.update', $music), [])
        ->assertForbidden();
});
```
