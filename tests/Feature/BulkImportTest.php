...
test('can create bulk import record', function () {
    ...
    expect($record->order_number)->toBe(1);
});

test('order_number is cast to integer', function () {
    $record = BulkImport::create([
        'collection' => 'test',
        'piece' => 'Test piece',
        'order_number' => '5', // string
    ]);

    expect($record->order_number)->toBeInt();
    expect($record->order_number)->toBe(5);
});
